...
export default defineEventHandler(async (event) => {
  ...
  const {
    ...
    fields: rawFields,
    locale,
    isCollection: rawIsCollection,
    populates: rawPopulates,
  } = query
  ...
  const populates = rawPopulates
    ? Array.isArray(rawPopulates)
      ? rawPopulates
      : [rawPopulates]
    : []
  ...
  try {
    ...
    if (isCollection) {
      ...
      return fields.length > 0 ? dataArray.map((item) => pickFields(item, fields)) : dataArray
    }
